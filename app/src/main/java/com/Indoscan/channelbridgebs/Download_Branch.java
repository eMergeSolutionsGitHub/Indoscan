 ...
 */
public class Download_Branch extends AsyncTask<String, Integer, Integer> {

    private final Context context;
    ProgressDialog dialog;

    public Download_Branch(Context context) {
    ...
    protected void onPostExecute(Integer returnCode) {
        ...
        if (returnCode == 2) {
            Toast notificationToast = Toast.makeText(this.context, "Download  Branch from the server.", Toast.LENGTH_SHORT);
            notificationToast.show();


        }
        if (returnCode == 1) {
            Toast notificationToast = Toast.makeText(this.context, "Unable to Download  branch from the server.", Toast.LENGTH_SHORT);
            notificationToast.show();
        }
        if (returnCode == 3) {
            Toast notificationToast = Toast.makeText(this.context,
                    "O size branch Download from the server", Toast.LENGTH_SHORT);
            notificationToast.show();
        }
