 ...
 */
public class Download_Master_Banks extends
    ...
    protected void onPreExecute() {
        ...
        dialog.setMessage("Fetching Master bank  Data from Server...");
    ...
    protected Integer doInBackground(String... strings) {
        ...
        Master_Banks PaymentType = new Master_Banks(context);
                ...
                try {
                    ...
                    if (repStoreDataResponse.size() > 0) {
                        ...
                        for (int i = 0; i < repStoreDataResponse.size(); i++) {
                            ...
                            PaymentType.openWritableDatabase();
                            Long result = PaymentType.insert_Master_Banks(DEL_Sales[0], DEL_Sales[1],
    ...
    protected void onPostExecute(Integer returnCode) {
        ...
        dialog.dismiss();
        if (returnCode == 2) {
            Toast notificationToast = Toast.makeText(this.context, "Download  Master bank from the server.", Toast.LENGTH_SHORT);
            notificationToast.show();

        }
        if (returnCode == 1) {
            Toast notificationToast = Toast.makeText(this.context, "Unable to Download  Master bank from the server.", Toast.LENGTH_SHORT);
            notificationToast.show();
        }
        if (returnCode == 3) {
            Toast notificationToast = Toast.makeText(this.context,
                    "O size Master Bank Download from the server", Toast.LENGTH_SHORT);
            notificationToast.show();
        }
    }
