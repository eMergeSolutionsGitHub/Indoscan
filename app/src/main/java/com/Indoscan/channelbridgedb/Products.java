...
//		cv.put(KEY_CODE, code);
    ...
    public ArrayList<Product> getProductsByPricipleAndCategory(String principle,String category) {
        ...
        Cursor cursor = database.rawQuery("select p.code,p.brand,p.unit_name,p.unit_size,p.generic_name,p.category,p.pro_des,p.introduced_date,p.principle,prs.row_id,prs.product_id,prs.batch_number,prs.quantity,prs.expiry_date,prs.price_purchase,prs.price_selling,prs.price_retail,prs.timestamp from products p  inner join productRepStore prs on p.code = prs.product_code  And principle = ? and category = ? GROUP BY p.code", new String[]{principle, category});
...
/*
    public ArrayList<Product> getProductsByPricipleAndCategory(String principle,String category) {
        ArrayList<Product> products = new ArrayList<>();
     //   Cursor cursor = database.rawQuery("select * from products  inner join productRepStore  on products.code = productRepStore.product_code  And principle = ? and category = ? GROUP BY `code`", new String[]{principle, category});
      Cursor cursor = database.rawQuery("select * from products  inner join productRepStore  on products.code = productRepStore.product_code  And principle = ? and category = ? ", new String[]{principle, category});

        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            Product productData = new Product();
            productData.setRowId(Integer.parseInt(cursor.getString(21)));// rowId of repstore
